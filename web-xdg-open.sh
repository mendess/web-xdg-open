...
__try() {
    ...
    case "$clean_link" in
        ...
        *.tar | *.bz2 | *.rar | *.Z | *.7z | *.gz | *.xz | *.zip)
            archive "$target"
            ;;
        *youtube.com* | *youtu.be* | *vimeo* | *clips.twitch.tv* | https://v.redd.it* | https://t.co* | *medal.tv* | *streamable.com* | *.avi | *.webm | *.mp4 | *.mp3 | *.wav | *.flac | *.midi | *.dvdrip | *.cam | *.mkv | *.mov | *.mpeg | *.flv | *.mpg | *.mp2 | *.mpv | *.m4p | *.m4v | *.wmv | *.qt | *.swf | *.avchd | *.m4a | *.ogg | *.wma | *.amv | *.mpa | *.ra | *.rax | *.raw | *.smf | *.snd | *.sng | *.swa | *.hma | *.aac | *.ac3 | *.eac3 | *.Vorbis | *.pcm)
            video "$target"
            ;;
        *.pub)
            clipboard "$target"
